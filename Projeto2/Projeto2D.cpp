...
void readInput(){
    ...
    */
    ...
    residual = vector<vector<edge>>(n+2);
    ...
    for (int i = 1; i <= n; ++i){
        ...
        rX1.id = 0;     rX2.id = i;
        rX1.revEdgeIndex = residual[0].size();
        rX2.revEdgeIndex = residual[i].size();
        
        rY1.flux = y;   rY2.flux = y;
        rY1.id = n+1;   rY2.id = i;
        rY1.revEdgeIndex = residual[n+1].size();
        rY2.revEdgeIndex = residual[i].size();

        residual[i].push_back(rX1);   //Aresta do processo i para processador X
        ...
        residual[n+1].push_back(rY2); //Aresta do processador Y para processo i
    }
...
bool BFS(int s, int t){
    ...
    while(!Q.empty()){
        int w = Q.front();    
        Q.pop();
        for(unsigned int i = 0; i < residual[w].size(); ++i){
...
int EdmondsKarp(){
    ...
    while(BFS(0,n+1)){
        ...
        minimum = INF;
        while (parent >= 0){                  
            minimum = min(minimum,residual[parent][parentEdge].flux);
            path.push(child);                 
            child = parent;
            parent = parents[child];
            parentEdge = parentEdges[child];
        ...
        while(path.size() > 1){
            ...
            curEdge = residual[cur][nextEdge].revEdgeIndex;
            residual[cur][nextEdge].flux -= minimum;
            residual[next][curEdge].flux += minimum;
        }
    }
    return total;
}
